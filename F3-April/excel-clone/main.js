...
for(let row = 0; row<rows; row++){
    ...
    for(let column = 0; column<columns;column++){
         ...
         td.setAttribute("id",`${String.fromCharCode(65 + column)}${row+1}`)
...
function onFocusFnc(event){
    ...
    document.getElementById("curr-cell").innerText = event.target.id;
...
document.getElementById("add-sheet-btn").addEventListener("click", () => {
  ...
  /*
  let btn = document.getElementById("new-Sheet");
  btn.innerHTML += `<button id="Sheet-${sheetCount}"> Sheet ${sheetCount++} </button> `
  console.log(sheetCount)*/
  let btn = document.createElement("button");
  btn.setAttribute("id",`sheet-${sheetCount}`)
  btn.innerText = `Sheet-${sheetCount++}`
 // btn.setAttribute("onclick","sheetDetail()")
  btn.addEventListener("focus", (event) => onFocusBtn(event));
  document.getElementById("new-Sheet").appendChild(btn)
...
var currentSheet ;
function onFocusBtn(event){
  console.log("In focus:", event.target);
  currentSheet = event.target;
  document.getElementById("curr-sheet").innerText = event.target.id;
  //console.log(currCell.style.cssText);
 // console.log(currCell.innerText);
  console.log(currentSheet.id);
}
/*
